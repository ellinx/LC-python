...
class Solution:
    def countOfAtoms(self, formula: str) -> str:
        ...
        l, r = 0, 0
        while l<len(formula):
            if formula[l].isupper():
                r = l+1
                while r<len(formula) and formula[r].islower():
                    r += 1
                element = formula[l:r]
                l = r
                while r<len(formula) and formula[r].isdigit():
                    r += 1
                if l<r:
                    counter[element] += int(formula[l:r])
                    l = r
                else:
                    counter[element] += 1
            elif formula[l]=="(":
                ...
                l += 1
                repeat = 1
                if formula[l].isdigit():
                    r = l+1
                    while r<len(formula) and formula[r].isdigit():
                        r += 1
                    repeat = int(formula[l:r])
                    l = r
                curCounter, counter = counter, stk.pop()
                for k in curCounter:
                    counter[k] += curCounter[k]*repeat
        ret = []
        for k in sorted(counter.keys()):
            ret.append(k)
            if counter[k]>1:
                ret.append(str(counter[k]))
        return "".join(ret)
