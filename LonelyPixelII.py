...
and a positive integer N, find the number of black pixels located at some specific row R and
...
class Solution:
    def findBlackPixel(self, picture, N):
        ...
        for i in range(m):
            for j in range(n):
                if picture[i][j]=='B' and len(rows[i])==N and len(cols[j])==N:
                    allSame = True
                    for k in cols[j]:
                        if k!=i:
                            sameRow[(i,k)] = sameRow.get((i,k), picture[i]==picture[k])
                            if not sameRow[(i,k)]:
                                allSame = False
                                break
                    if allSame:
