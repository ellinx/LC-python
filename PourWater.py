...
class Solution:
    def pourWater(self, heights, V, K):
        ...
        def goLeft(heights, K):
            cur = K
            # if left neighbor has same height then try go left
            if cur and heights[cur-1]==heights[cur]:
                temp = goLeft(heights, cur-1)
                if temp == cur-1:
                    return cur
                return temp
            # just find lowest most left
            while cur and heights[cur-1]<=heights[cur]:
                cur -= 1
            # need to go back to lowest rightmost index
            if cur!=K:
                while heights[cur]==heights[cur+1]:
                    cur += 1
            return cur
        def goRight(heights, K):
            cur = K
            # if right neighbor has same height then try go right
            if cur+1<len(heights) and heights[cur+1]==heights[cur]:
                temp = goRight(heights, cur+1)
                if temp == cur+1:
                    return cur
                return temp
            # just find lowest most right
            while cur+1<len(heights) and heights[cur+1]<=heights[cur]:
                cur += 1
            # need to go back to lowest leftmost index
            if cur!=K:
                while heights[cur]==heights[cur-1]:
                    cur -= 1
            return cur
        ...
        while V:
            index = goLeft(heights,K)
            if index!=K:
                heights[index] += 1
                V -= 1
                continue
            index = goRight(heights,K)
            if index!=K:
                heights[index] += 1
                V -= 1
                continue
